...
4.3. ESTRATEGIA DE ACTUALIZACIÓN AUTOMÁTICA (CUMPLIMIENTO DEL OKR)
------------------------------------------------------------------
Para mantener este inventario actualizado y cumplir el objetivo de reducción de 
librerías obsoletas, se ha implementado la siguiente estrategia automatizada 
en el repositorio:

1. Herramienta de Monitorización: 
   Se ha configurado GitHub Dependabot mediante el archivo .github/dependabot.yml.

2. Frecuencia de Análisis: 
   El sistema escanea semanalmente los archivos de definición de dependencias 
   (pom.xml para backend y package.json para frontend).

3. Remediación Automática:
   - Si Dependabot detecta una versión obsoleta o con vulnerabilidades de seguridad 
     conocidas (CVEs), genera automáticamente una Pull Request.
   - Esta PR incluye las "Release Notes" de la librería y el puntaje de compatibilidad, 
     facilitando la decisión de actualización al equipo de desarrollo.

RESULTADO:
Esta implementación garantiza que la deuda técnica asociada a dependencias externas 
se mantenga al mínimo, asegurando la estabilidad y seguridad del proyecto a largo plazo.
